...
/// <returns></returns>
bool SceneStage2::AddShot(ShotBase* pShot)
{
	// nullptrを渡されたら止まる
	assert(pShot);
...
// HP小回復
void SceneStage2::DropHpSmallRecovery(int enemyIndex)
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
// HP大回復
void SceneStage2::DropHpGreatRecovery(int enemyIndex)
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
// 弾小回復
void SceneStage2::DropShotSmallRecovery(int enemyIndex)
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
// 弾大回復
void SceneStage2::DropShotGreatRecovery(int enemyIndex)
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
// 残機回復
void SceneStage2::DropLifeRecovery(int enemyIndex)
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
// HP全回復
void SceneStage2::DropFullHpRecovery()
{
	for (int i = 0; i < m_pRecovery.size(); i++)
	{
		if (!m_pRecovery[i])
...
/// </summary>
/// <param name="enemyIndex">敵</param>
void SceneStage2::CreateItem(int enemyIndex)
{
...
/// 武器切り替え画面表示
/// </summary>
void SceneStage2::DrawShotChange()
{
	// 現在のHPを表示
...
/// スタート演出の描画
/// </summary>
void SceneStage2::DrawStartStaging()
{
	// フェードイン→アウト
...
/// クリア演出の描画
/// </summary>
void SceneStage2::DrawClearStaging()
{
	// タイム
