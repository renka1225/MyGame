...
public:
	...
	// 演算子オーバーロード
	static Quaternion operator*(Quaternion lQ, Quaternion rQ)
	{
	...
	static VECTOR operator*(Quaternion qRot, VECTOR right)
	{
		//3次元座標をクオータニオンに変換
		...
		//回転後のクオータニオンの作成
		qPos.w = qRot.w * qPos.w * qInv.w;
		qPos.x = qRot.x * qPos.x * qInv.x;
		qPos.y = qRot.y * qPos.y * qInv.y;
		qPos.z = qRot.z * qPos.z * qInv.z;
